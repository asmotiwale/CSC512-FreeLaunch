    ...
    public:
          cudaSubkernelCallHandler(Rewriter &Rewrite) : Rewrite(Rewrite) {}
            ...
            virtual void run(const MatchFinder::MatchResult &Result) {
                ...
                replaceAll(strTrimmed,"threadIdx.x + blockIdx.x*blockDim.x","kkk * FL_childBlockSize + threadIdx.x%FL_childBlockSize");
                replaceAll(strTrimmed,"threadIdx.x+ blockIdx.x*blockDim.x","kkk * FL_childBlockSize + threadIdx.x%FL_childBlockSize");
                replaceAll(strTrimmed,"threadIdx.x +blockIdx.x*blockDim.x","kkk * FL_childBlockSize + threadIdx.x%FL_childBlockSize");
                replaceAll(strTrimmed,"threadIdx.x+blockIdx.x*blockDim.x","kkk * FL_childBlockSize + threadIdx.x%FL_childBlockSize");
...
class cudaSubKernelReturnCallHandler : public MatchFinder::MatchCallback {
        public:
                 ...
                 Rewrite.ReplaceText(SourceRange(returnstmt->getLocStart(),returnstmt->getLocEnd()),"goto P");
              }
        private:
              Rewriter &Rewrite;
