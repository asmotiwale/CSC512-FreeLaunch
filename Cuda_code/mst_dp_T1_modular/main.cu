...
__global__ void verify_min_elem(unsigned *mstwt, unsigned nnodes,unsigned nedges,unsigned *noutgoing,unsigned *nincoming,unsigned *edgessrcdst,foru *edgessrcwt, unsigned *srcsrc,unsigned *psrc,unsigned *ele2comp, foru *eleminwts, foru *minwtcomponent, unsigned *partners, unsigned *phore, bool *processinnextiteration, unsigned *goaheadnodeofcomponent, unsigned inpid,int blocks,char *FL_Args) {
  ...
  if (id < nnodes) {
	...
	FL_childKernelArgSz = sizeof(int)+sizeof(unsigned int)+sizeof(unsigned int)+sizeof(foru)+sizeof(_Bool)+sizeof(unsigned int);
	...
	_tmp_p+=sizeof(int);
	FL_childBlockSize = 32;
	memcpy((void*)_tmp_p, (void*) &degree, sizeof(unsigned int));
_tmp_p+=sizeof(unsigned int);
	memcpy((void*)_tmp_p, (void*) &id, sizeof(unsigned int));
_tmp_p+=sizeof(unsigned int);
	memcpy((void*)_tmp_p, (void*) &minwt, sizeof(foru));
_tmp_p+=sizeof(foru);
	memcpy((void*)_tmp_p, (void*) &minwt_found, sizeof(_Bool));
_tmp_p+=sizeof(_Bool);
	memcpy((void*)_tmp_p, (void*) &minwt_node, sizeof(unsigned int));
	atomicAdd(&FL_totalBlocks,_tmp_childGridSize);
	FL_check = FL_ttid;
	goto P;
		C:   __threadfence(); // only the last two arguments were modified before this call within this function
	...
	_tmp_p+=sizeof(int);
	unsigned int degree;
	memcpy((void*)&degree, (void*)_tmp_p, sizeof(unsigned int));
	_tmp_p+=sizeof(unsigned int);

	unsigned int id;
	memcpy((void*)&id, (void*)_tmp_p, sizeof(unsigned int));
	_tmp_p+=sizeof(unsigned int);
	...
	_Bool minwt_found;
	memcpy((void*)&minwt_found, (void*)_tmp_p, sizeof(_Bool));
	_tmp_p+=sizeof(_Bool);

	unsigned int minwt_node;
	memcpy((void*)&minwt_node, (void*)_tmp_p, sizeof(unsigned int));

  unsigned ii = logicalChildBlockSeqNum * FL_childBlockSize + threadIdx.x%FL_childBlockSize;
  if(ii<degree){
